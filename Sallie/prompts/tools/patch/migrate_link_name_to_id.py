...
def main(argv: List[str]) -> int:
    ...
    if args.clean_up_dangling_links:
        ...
        # Concise, elegant final summary line
        print(
            f"Summary \u2713 Cleaned links in {rules_path.name}: kept {total_after}/{total_before}, removed {count_removed}. Report: {sidecar.name}"
        )
    ...
    if strict_fail:
        ...
        print(
            f"Summary \u2717 Issues in {rules_path.name}: scanned {total_links}, updated {changed_links}, unresolved {len(unresolved)}, ambiguous {len(ambiguous)}. See: {sidecar.name}"
        )
        return 2
    ...
    # Concise, elegant final summary line
    print(
        f"Summary \u2713 Updated links in {rules_path.name}: scanned {total_links}, updated {changed_links}, unresolved {len(unresolved)}, ambiguous {len(ambiguous)}. Report: {sidecar.name}"
    )
