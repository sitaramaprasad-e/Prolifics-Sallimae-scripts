...
def print_help() -> None:
    ...
    print("      - logicIds from that snapshot, merged into the model")
...
def load_business_names(json_dir: Path) -> dict[str, str]:
    """
    ...
    if not path.exists():
        print(f"Warning: business_rules.json not found at {path}; "
              "logicIds will be shown without names.")
        return {}
    ...
    # Support both legacy list and rooted {"version", "logics"} structures
    if isinstance(raw, dict) and isinstance(raw.get("logics"), list):
        data = raw["logics"]
    elif isinstance(raw, list):
        data = raw
    else:
        print("Warning: business_rules.json is not a list or an object with a 'logics' array; "
              "logicIds will be shown without names.")
        return {}
    ...
    for logic in data:
        ...
        name = logic.get("heading") or logic.get("name")
        if not name:
            continue
        id_to_name[rid] = name
...
def load_logic_archived_flags(json_dir: Path) -> dict[str, bool]:
    """
    ...
    for logic in data:
        ...
        if not rid:
            continue
        flags[rid] = bool(logic.get("archived", False))
    return flags
...
def choose_hierarchy(model: dict, id_to_name: dict[str, str]) -> int:
    ...
    for i, h in enumerate(hierarchies):
        ...
        top_val = get_top_id(h)
        top = top_val if top_val is not None else "<no-topId>"
        use_graph = h.get("useGraph")
        top_name = id_to_name.get(top) if top and top != "<no-topId>" else None
        if top_name:
            top_display = f"{top} ({top_name})"
        else:
            top_display = top
        extra = f", useGraph={use_graph}" if use_graph is not None else ""
        print(f"  [{i+1}] {name} (topId={top_display}{extra})")
...
# === Helper functions for normalizing logicIds/businessLogicIds and topId/topDecisionId ===
def get_logic_ids(model: dict) -> list[str]:
    """Return logic IDs from either logicIds or businessLogicIds, normalizing to a list."""
    ids = model.get("logicIds")
    if isinstance(ids, list):
        return ids
    ids = model.get("businessLogicIds")
    if isinstance(ids, list):
        return ids
    return []


def set_logic_ids(model: dict, ids: list[str]) -> None:
    """Set logic IDs on both logicIds and businessLogicIds for compatibility."""
    model["logicIds"] = ids
    model["businessLogicIds"] = ids


def get_top_id(hierarchy: dict) -> str | None:
    """Return the top decision id from either topId or topDecisionId, if present."""
    top = hierarchy.get("topId")
    if top:
        return top
    top = hierarchy.get("topDecisionId")
    if top:
        return top
    return None
...
def find_matching_hierarchy(
    ...
    current_top = get_top_id(current_h)
    current_name = current_h.get("name")

    # 1) Match by topId/topDecisionId
    if current_top:
        for h in backup_hiers:
            if get_top_id(h) == current_top:
                return h
...
def print_business_logic_ids_with_names(backup_model: dict, current_model: dict, id_to_name: dict[str, str], archived_flags: dict[str, bool]) -> None:
    backup_ids = get_logic_ids(backup_model)
    current_ids = get_logic_ids(current_model)
    if not backup_ids and not current_ids:
        print("\nNo logicIds found for this model (backup or current).")
        return
    if not backup_ids:
        print("\nNo logicIds found for this model in the backup snapshot.")
    else:
    ...
    # First, list all logicIds from the backup, marking those not present in the current model
    for bid in backup_ids:
        ...
        if not name:
            annotations.append("WARNING: no matching logic in business_rules.json")
        elif archived:
            annotations.append("WARNING: logic is archived")
    ...
    # Then, list any logicIds that are in the current model but missing in this backup snapshot
    missing_in_backup = current_ids_set - backup_ids_set
    if missing_in_backup:
        print(
            f"\nLogicIds present in current model but missing from this backup: {len(missing_in_backup)} "
...
def restore_hierarchy(
    ...
    models_version: int | None,
):
    # Make a backup before modifying
    make_backup(models_path)

    # Best-effort optimistic concurrency: if we have a version, ensure it hasn't changed on disk
    if models_version is not None and models_path.exists():
        try:
            with models_path.open("r", encoding="utf-8") as f:
                cur_raw = json.load(f)
            if isinstance(cur_raw, dict) and isinstance(cur_raw.get("models"), list):
                ...
                if isinstance(cur_version, int) and cur_version != models_version:
                    print(
                        ...
                        f"aborting restore to avoid overwriting concurrent changes."
                    )
                    return
        except Exception as ex:
            print(f"[WARN] Could not re-read models.json for concurrency check; proceeding anyway: {ex}")
    ...
    # Merge logicIds from backup model into current model (union, no dupes, only valid ids)
    current_ids = get_logic_ids(model)
    backup_ids = get_logic_ids(backup_model)
    ...
    set_logic_ids(model, merged_ids)

    # Normalize topId on the hierarchy we are about to write back
    top_id_val = get_top_id(new_hierarchy)
    if top_id_val is not None:
        new_hierarchy["topId"] = top_id_val
    ...
    if isinstance(models_root, dict):
        ...
        to_write = {"version": new_version, "models": models_current}
...
def main():
    ...
    print("NOTE: Restore will update both the selected hierarchy and the model's logicIds.")
    ...
    business_names = load_business_names(json_dir)
    logic_archived = load_logic_archived_flags(json_dir)
    valid_business_logic_ids: set[str] = set(business_names.keys())

    try:
        models_raw = load_models(models_path)
    except Exception as e:
        print(f"Error loading models.json: {e}")
        return

    # Support both legacy list and rooted {"version", "models"} structures
    models_root = None
    models_version: int | None = None
    if isinstance(models_raw, dict) and isinstance(models_raw.get("models"), list):
        models_root = models_raw
        mv = models_raw.get("version")
        if isinstance(mv, int):
            models_version = mv
        models_current = models_raw["models"]
    elif isinstance(models_raw, list):
        models_current = models_raw
    else:
        print("models.json is not in a supported format; aborting.")
        return
    ...
    if choose_current == "y":
        hierarchy_index: int | None = choose_hierarchy(model_current, business_names)
    else:
    ...
    while 0 <= current_backup_pos < len(backups):
        ...
        # Support both legacy list and rooted {"version", "models"} structures in backups
        if isinstance(raw_backup, dict) and isinstance(raw_backup.get("models"), list):
            models_backup = raw_backup["models"]
        elif isinstance(raw_backup, list):
            models_backup = raw_backup
        else:
            print("  Backup file does not contain a list of models or an object with a 'models' array; skipping.")
            cmd = input("Command: [n]ext (older), [p]revious (newer), [q]uit: ").strip().lower() or "n"
            if cmd == "n":
                current_backup_pos += 1
            elif cmd == "p":
        ...
        if hierarchy_index is not None:
            ...
            # Show the logicIds for this backup model, with names from business_rules.json,
            # and mark differences vs the current model.
            print_business_logic_ids_with_names(backup_model, model_current, business_names, logic_archived)

            # Cross-reference topId/topDecisionId and print its name if available, with warnings
            top_id = get_top_id(backup_hierarchy)
            if top_id:
                top_name = business_names.get(top_id)
                top_archived = logic_archived.get(top_id, False)
                if not top_name:
                    print(f"\nTop: {top_id} (name not found) [WARNING: no matching logic in business_rules.json]")
                elif top_archived:
                    print(f"\nTop: {top_id} ({top_name}) [WARNING: logic is archived]")
                else:
                    print(f"\nTop: {top_id} ({top_name})")
            else:
                print("\nTop: <none>")
            ...
            if not backup_hiers:
                ...
                for idx, h in enumerate(backup_hiers, start=1):
                    ...
                    top_val = get_top_id(h)
                    top = top_val if top_val is not None else None
                    top_name = business_names.get(top) if top else None
                    top_archived = logic_archived.get(top, False) if top else False
                    annotations: list[str] = []
                    if top:
                        if not top_name:
                            annotations.append("WARNING: no matching logic in business_rules.json")
                        ...
                        top_display = "<no-topId>"
                    ann_str = f" [{' | '.join(annotations)}]" if annotations else ""
                    print(f"  [{idx}] {h_name} (topId={top_display}){ann_str}")
