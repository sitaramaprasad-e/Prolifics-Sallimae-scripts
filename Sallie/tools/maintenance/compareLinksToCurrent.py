...
def print_help() -> None:
    ...
    print()
    print("For each backup snapshot, it shows per business rule:")
    ...
    print("  * Only rules with differences are listed")
...
def load_business_rules(path: Path):
    ...
    if not isinstance(data, list):
        raise ValueError("business_rules.json is not a list of rules.")
    return data
...
def canonical_link(link: dict) -> tuple:
    ...
    return (
        ...
        link.get("to_input"),
        link.get("kind"),
        link.get("from_step_id"),
    )


def summarize_link(link: dict, from_rule_name=None) -> str:
    """
    Produce a concise single-line string describing a link, optionally
    including the name of the rule referenced by from_step_id.
    """
    base = (
        f"{link.get('from_output')} -> {link.get('to_input')} "
        f"(kind={link.get('kind')}, from={link.get('from_step_id')}"
    )
    if from_rule_name:
        base += f" [{from_rule_name}]"
    base += ")"
    return base
...
def build_rule_map(rules: list) -> dict:
    """
    Build a mapping of rule_id -> rule_object for quick lookup.
    """
    rule_map = {}
    for r in rules:
...
def diff_links_for_backup(
    ...
    # Build a combined id -> name map for cross-referencing from_step_id
    ...
    print(f"Rules with link differences vs current: {len(changed_rules)}")
...
def main():
    ...
    try:
        current_rules = load_business_rules(rules_path)
    ...
    while 0 <= current_backup_pos < len(backups):
        ...
        if not isinstance(backup_rules, list):
            print("  Backup file does not contain a list of rules; skipping.")
            cmd = input("Command: [n]ext, [p]revious, [q]uit: ").strip().lower() or "n"
            if cmd == "n":
                current_backup_pos += 1
            elif cmd == "p":
