...
def _select_spec_file(default_dir: str) -> Optional[dict]:
    print(f"Select a spec file from {default_dir}")
    try:
        files = [f for f in os.listdir(default_dir) if f.endswith(".json")]
    except Exception:
        print("  (No spec directory or unable to read it)")
        return None
    if not files:
        print("  (No JSON spec files found)")
        return None
    ...
    try:
        ...
        if idx < 1 or idx > len(files):
            return None
        path = os.path.join(default_dir, files[idx-1])
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    spec_dir = os.path.join(model_home, "specs")
    spec = _select_spec_file(spec_dir)
