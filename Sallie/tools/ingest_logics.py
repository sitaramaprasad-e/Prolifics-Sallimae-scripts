...
import sys
import os
 # Ensure project root is on sys.path so `tools` can be imported when this
# script is invoked via an absolute path (python .../tools/ingest_logics.py).
...
import hashlib
import shutil
from tools.helpers.logic_ingest_core import (
    add_logics_from_text,
    heading_text,
    set_graph_disabled,
    _populate_graph_for_logic,
    set_graph_base_url,
)
...
LOG = logging.getLogger("ingest_logics")
...
LOG.info("[info] ingest_logics starting • MODEL_HOME=%s", MODEL_HOME)
...
if len(sys.argv) < 2:
    print("Usage: python ingest_logics.py <input_file> [--force | --force-load]")
    sys.exit(1)
...
if FORCE_LOAD:
    LOG.info("[info] Force-load enabled: ingesting even if (name,timestamp) already exists")

# Optional mode: ingest logics from previous iterations of the same report (via PCPT logs)
ALL_RUNS = any(arg == "--all-runs" for arg in sys.argv[2:]) or \
           (os.environ.get("PCPT_INGEST_ALL_RUNS", "").lower() in {"1", "true", "yes"})
if ALL_RUNS:
    LOG.info("[info] All-runs mode: will also ingest logics from previous iterations via PCPT logs")
...
# Use a timezone-aware UTC datetime to avoid deprecation warnings
_dt = datetime.fromtimestamp(file_mtime, timezone.utc).replace(microsecond=0)
file_timestamp = _dt.isoformat().replace("+00:00", "Z")
...
def _extract_names(doc_text: str) -> List[str]:
    ...
    for sec in parts[1:]:
        ...
        nm = heading_text(first)  # from logic_ingest_core
...
def write_model_sources_and_runs(logic_ids_for_output: Optional[List[str]] = None,
                                 output_file_path: Optional[str] = None,
                                 link_all: bool = False):
    """Scan logs/reports for PCPT headers and emit .model/sources.json and .model/runs.json.
    If `logic_ids_for_output` and `output_file_path` are provided, attach the list of rule IDs
    ...
    if output_file_path and os.path.exists(output_file_path):
        try:
            ...
            names_from_doc = _extract_names(raw)
        except Exception:
            doc_norm = ""
            names_from_doc = []
    # Preserve previously stored logic_ids from existing runs.json before we add new links
    try:
        _existing_runs = []
    ...
    # Helper to preserve previous logic_ids after linking, but only if still empty
    def _preserve_previous_links_when_empty():
        for rec in runs:
            k = _run_key(rec)
            prev = _existing_map.get(k)
            if prev and isinstance(prev, dict):
                prev_ids = prev.get("logic_ids")
                if prev_ids and not rec.get("logic_ids"):
                    try:
                        rec["logic_ids"] = list(prev_ids)
                    except Exception:
                        rec["logic_ids"] = prev_ids
                    LOG.info("[info] preserved prior logic_ids for run ts=%s log=%s (count=%d)", rec.get("timestamp"), rec.get("log_file"), len(prev_ids))
    ...
    if logic_ids_for_output and output_file_path:
        try:
            ids_list = list(logic_ids_for_output)
        except Exception:
            ids_list = None
        if ids_list:
            matched_idxs = []
            ...
            if not matched_idxs and names_from_doc:
                ...
                if scored:
                    ...
                    # 1. Load rule_id → name mapping for ids_list from business_rules.json
                    id_to_name = {}
                    try:
                    ...
                    # 2. For each run in chosen, collect the subset of ids_list whose name is found in response_text
                    found_ids_per_run = []
                    found_logic_ids = set()
                    for tup in chosen:
                        idx = tup[0]
                        ...
                        for rid in ids_list:
                            ...
                            if not name:
                                continue
                            if (f"## {name}" in resp) or (name in resp):
                                matched_logic_ids.append(rid)
                        if matched_logic_ids:
                            found_logic_ids.update(matched_logic_ids)
                            # Attach only these rule IDs to this run, de-duped
                            existing = rec.get("logic_ids") or []
                            combined = list(existing) + [rid for rid in matched_logic_ids if rid not in set(existing)]
                            runs[idx]["logic_ids"] = combined
                            LOG.info("[info] link rules → run: idx=%d ts=%s log=%s ids=%d (partial per-rule match)", idx, rec.get("timestamp"), rec.get("log_file"), len(matched_logic_ids))
                            _audit_add("link", path=str(output_file_path), source=str(rec.get("log_file")), decision="accepted", reason="partial per-rule match", tests={"matched": True}, derived={"logic_ids": matched_logic_ids})
                        found_ids_per_run.append((idx, matched_logic_ids))
            ...
            if matched_idxs:
                ...
                for idx in target_indices:
                    existing = runs[idx].get("logic_ids") or []
                    try:
                    ...
                    _audit_add("link", path=str(output_file_path), source=str(runs[idx].get("log_file")), decision="accepted", reason=("partial content match" if partial_mode else "content match"), tests={"matched": True}, derived={"logic_ids": ids_list})
                    runs[idx]["logic_ids"] = combined
    _preserve_previous_links_when_empty()
    os.makedirs(os.path.dirname(SOURCE_JSON), exist_ok=True)
    _save_json_file(SOURCE_JSON, sources_out)
...
# Track existing rules file metadata for optimistic concurrency
existing_rules_mtime = None
existing_version = 0

# Read existing rules if output file already exists and is not empty
if os.path.exists(output_file) and os.path.getsize(output_file) > 0:
    existing_rules_mtime = os.path.getmtime(output_file)
    ...
    if isinstance(raw, dict):
        # New structure: { "version": <int>, "logics": [ ... ] }
        try:
            existing_version = int(raw.get("version") or 0)
        except Exception:
            existing_version = 0
        logics = raw.get("logics")
        if isinstance(logics, list):
            existing_rules = logics
        else:
            LOG.warning("[warn] %s has dict structure but no valid 'logics' list; starting fresh.", output_file)
            existing_rules = []
    elif isinstance(raw, list):
        # Legacy structure: bare list of rules
        existing_rules = raw
        existing_version = 0
    else:
        LOG.warning("[warn] %s has unexpected JSON structure (%s); starting fresh.", output_file, type(raw).__name__)
        existing_rules = []
else:
    existing_rules = []
    existing_rules_mtime = None
...
existing_by_name = {
    ...
    if "name" in r and "timestamp" in r
}
...
# Build set of seen (name, timestamp) pairs from existing rules (normalized)

seen = set()
for rule in existing_rules:
    k = _dedupe_key(rule.get("name"), rule.get("timestamp"))
    if k:
        seen.add(k)
...
if KG_ONLY:
    ...
    for r in rules_for_kg:
        try:
            _populate_graph_for_logic(r, LOG)
            processed += 1
        except Exception as e:
            LOG.warning(
...
_ingest_owner = _prompt_with_default("Enter team/owner to set on all logics", default_owner)
_ingest_component = _prompt_with_default("Enter component to set on all logics", default_component)
...
with open(input_file, "r", encoding="utf-8") as f:
    new_logics_from_doc, updated_count, new_count, considered_count = add_logics_from_text(
        f.read(),
        file_timestamp,
    ...
    new_logics.extend(new_logics_from_doc)
...
if ALL_RUNS:
    ...
    _current_names  = {r.get("name") for r in new_logics if r.get("name")}
    _allowed_names  = _existing_names.union(_current_names)
    for ts, resp_text in prior_items:
        prior_rules, updated_count, new_count, considered_count = add_logics_from_text(
            resp_text,
        ...
        new_logics.extend(prior_rules)

final_rules = {r["name"]: r for r in existing_rules}
for r in new_logics:
    final_rules[r["name"]] = r  # overwrite with latest

LOG.info("[step 3/4] Persisting rules to model…")

# Optimistic concurrency check: if the rules file has changed on disk since we read it,
# abort rather than overwriting someone else's newer changes (unless --force/--force-load).
if existing_rules_mtime is not None and not FORCE_LOAD:
    try:
        current_mtime = os.path.getmtime(output_file)
    except FileNotFoundError:
        current_mtime = None
    if current_mtime is not None and current_mtime != existing_rules_mtime:
        LOG.error(
            "[error] %s was modified on disk during ingest (mtime changed); "
            "aborting write. Re-run with --force to overwrite.",
            output_file,
        )
        print(
            f"ERROR: {output_file} changed on disk during ingest; "
            "re-run with --force to overwrite.",
            file=sys.stderr,
        )
        sidecar_path = _audit_write(prefix="rules_ingest_conflict")
        if sidecar_path:
            LOG.info("[info] Wrote audit sidecar: %s", sidecar_path)
        sys.exit(1)
...
wrapped = {
    "version": new_version,
    "logics": final_rules_list,
}
_save_json_file(output_file, wrapped)

LOG.info(
    "[info] Extracted %d rules: %d new, %d updated. Total now: %d. Saved to %s (version=%s). Considered %d section(s).",
    len(new_logics),
    new_count,
    updated_count,
    len(final_rules_list),
    output_file,
    new_version,
    considered_count,
)
...
_current_run_logic_ids = [r.get("id") for r in new_logics if r.get("id")]
LOG.info("[step 4/4] Rebuilding sources/runs index and linking this run…")
write_model_sources_and_runs(
    logic_ids_for_output=_current_run_logic_ids,
